...
11. As a user, I want the refresh button in the top-right of the caption row, so that it is visible and discoverable without disturbing the table structure.
...
- **Refresh button**: rendered in the top-right of the caption row — the native `<caption>` becomes a `flex justify-between` container with the title on the left and a small icon button (`aria-label="Refresh"`) on the right. It is rendered only in server mode, is `disabled` while a request is in flight, and re-fires the current request without resetting the page.
